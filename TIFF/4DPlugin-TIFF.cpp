...
static PA_long32 get_picture_index(PA_Picture p, const char *type) {
    ...
    if(p)
    {
        ...
        while (err == eER_NoErr)
        {
            ...
            {
                //convert to utf-8
                uint32_t len = (uint32_t)(type.fLength * 4) + sizeof(uint8_t);
                ...
                std::string uti = std::string((const char *)&buf[0]);
                //.tif;.tiff;image/tiff;image/x-tiff;TIFF
                for(pos = uti.find(';'); pos != std::string::npos; pos = uti.find(';', found))
                {
                    std::string typestring = uti.substr(found, pos-found);
                    found = pos + 1;
                    types.insert(std::map<std::string, uint32_t>::value_type(typestring, i));
                }
                std::string typestring = uti.substr(found, uti.length()-found);
                types.insert(std::map<std::string, uint32_t>::value_type(typestring, i));
            }
        }
        //look for .tif
...
void PluginMain(PA_long32 selector, PA_PluginParameters params) {
	...
	try
	{
        switch(selector)
        {
			...
			case 1 :
				TIFF_Count_pages(params);
				break;
			case 2 :
				TIFF_GET_PAGES(params);
				break;
			case 3 :
				TIFF_Create_from_array(params);
				break;
...
static tsize_t tiff_Read(thandle_t h, tdata_t buf, tsize_t size) {
...
static toff_t tiff_ReadSize(thandle_t h) {
...
static tsize_t tiff_Write(thandle_t h, tdata_t bytes, tsize_t size) {
...
static toff_t tiff_WriteSize(thandle_t h)
{
    tiff_dst *tiff_output = (tiff_dst *)h;
...
static void TIFF_Count_pages(PA_PluginParameters params) {
    ...
    tiff_src tiff_input;
    //$0
    PA_long32 pageCount = 0L;
    //$1
    PA_Picture p = PA_GetPictureParameter(params, 1);
    PA_long32 i = get_picture_index(p, "image/tiff");
    
    if(i)
    {
        PA_Handle h = PA_NewHandle(0);
        PA_GetPictureData(p, i, h);
        if(PA_GetLastError() == eER_NoErr)
        {
            tiff_input.ptr = (uint8_t *)PA_LockHandle(h);
            tiff_input.len = PA_GetHandleSize(h);
            tiff_input.pos = 0;
    ...
    PA_ReturnLong(params, pageCount);
}
...
static void TIFF_GET_PAGES(PA_PluginParameters params) {
        ...
        if(PA_GetLastError() == eER_NoErr)
        {
            ...
            if(tiff_input.len)
            {
                ...
                if(tiff)
                {
                    ...
                    int param_start = PA_GetLongParameter(params, 3);
                    int param_end = PA_GetLongParameter(params, 4);
                    
                    //negative position is relative to end
                    if(param_start < 0)
                    {
                        param_start = src_count_pages + param_start;
                        param_start = param_start > 0 ? param_start : 1;
                    }

                    if(param_end < 0)
                    {
                        param_end = src_count_pages + param_end;
                        param_end = param_end > 0 ? param_end : 1;
                    }
                    
                    int start = 1;
                    int end = src_count_pages;
                    
                    end = param_end < src_count_pages ? (param_end ? param_end : src_count_pages) : src_count_pages;
                    start = param_start < end ? (param_start ? param_start : start) : start;
                    
                    for (uint16 dir = start - 1; dir < end;++dir)
                    {
                        ...
                        if(page)
                        {
                            ...
                            if(Param2.fType == eVK_ArrayPicture)
                            {
                                ...
                                PA_SetPictureInArray(Param2, count, picture);
                            }
                        }
    ...
    //sync $2
    PA_Variable **pParams = (PA_Variable **)params->fParameters;
...
static void TIFF_Create_from_array(PA_PluginParameters params) {
    //$1
    PA_Variable Param1 = PA_GetVariableParameter(params, 1);
    ...
    if(Param1.fType == eVK_ArrayPicture)
    {
        ...
        if(result)
        {
            for(PA_long32 pos = 1;pos <= PA_GetArrayNbElements(Param1);++pos)
            {
                PA_Picture p = PA_GetPictureInArray(Param1, pos);
                PA_long32 i = get_picture_index(p, "image/tiff");
                if(i)
                {
                    TIFF *tiff = 0;
